...
// Migraciones de versiones anteriores de la base de datos.
function migrateDb() {
  const startVersion = db.version;

  // v1 -> v2: mesas numeradas -> locales/áreas/mesas, listas de precios, stock máximo.
  if (db.version < 2) {
    db.version = 2;
  ...
  // v2 -> v3: clave de supervisor, POS, envío de comandas por categoría, vigencia de tarifas,
  // producto de precio libre, descripción de mesas y líneas de comanda con id/categoría/estado.
  if (db.version < 3) {
    db.version = 3;
...
// Una lista de precios solo aplica dentro de su periodo de vigencia (fechas abiertas si null).
function plVigente(pl) {
  const today = todayISO();
  return (!pl.validFrom || today >= pl.validFrom) && (!pl.validUntil || today <= pl.validUntil);
}

// Precio de un producto según la tarifa de la comanda (o precio base si no hay tarifa vigente u override).
...
// (el cliente puede retractarse); al confirmarse pasa a 'pedido' y de ahí avanza
// por 'procesando' y 'listo' hasta 'entregado' (llevado al cliente: fin del ciclo).
...
// La clave de supervisor nunca se envía a los clientes.
...
const routes = {
  ...
  // Validar la clave de supervisor (acceso a Ajustes y acciones protegidas).
  ...
  // Ámbito: todos los productos, una categoría o un producto concreto.
  ...
  'DELETE /api/orders/:id': (req, res, body, params) => {
    ...
    for (const it of order.items) {
      const p = findProduct(it.productId);
      if (p && !p.openPrice) p.stock += it.qty; // anular comanda devuelve el stock
    }
    db.orders = db.orders.filter(o => o.id !== order.id);
    saveDb();
    jsonRes(res, 200, { ok: true });
  ...
  // Los productos de precio libre no se inventarían y quedan fuera.
